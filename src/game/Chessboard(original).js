...
const Chessboard = () => {
  ...
  async function makeAIMove() {
    ...
    try {
      ...
      stockfishEngine.postMessage("go depth 10"); // You can adjust the depth as needed
  ...
  function handleMovePiece(fromSquare, toSquare) {
    ...
    try {
      ...
      if (chess.isCheckmate()) {
        setErrorMessage("Checkmate! Game over.");
      } else if (chess.isStalemate()) {
        setErrorMessage("Stalemate! Game over.");
      } else if (halfmoveClock === 50) {
        setErrorMessage("Draw! Game over.");
